    ...
    try:
        ...
        if matched_container:
            ssh_mgr.execute_command(
                f"{cli_prefix}{cont_cli} logs {matched_container} --tail 100",
                append_log,
                check_exit_code=False,
            )
        ...
        if not is_running:
            ...
            diag_cmds = [
                ...
                f"{cont_cli} logs njorddeploy-web-notepad 2>&1",
            ]
...
def run_environment_tests(
    ...
    logger.info(f"Testing {len(target_components)} components...")
    logger.info("==================================================")
        ...
        for comp in target_components:
            ...
            try:
                if is_lxc:
                    ...
                    for cmd in [
                        f"{clean_cli} ps -q | xargs -r {clean_cli} stop "
                        "2>/dev/null || true",
                        f"{clean_cli} ps -aq | xargs -r {clean_cli} rm -f "
                        "2>/dev/null || true",
                    ...
                    proxmox_client.start_vm(node=node, vmid=vmid_val)
                    vm_ip = wait_for_ip(proxmox_client, node, vmid_val)
                    if not vm_ip:
                        raise TimeoutError("Unable to retrieve cloned VM IP address.")
                    test_record["vmid"] = new_vmid
                    test_record["ip"] = vm_ip
                    time.sleep(10)

                if not vm_ip:
                    raise RuntimeError("Target IP address could not be resolved.")
                ...
                user_vars["CONTAINER_ENGINE"] = engine
                ...
                # Deploy via Ansible
                logger.info(
                    f"Executing Ansible deployment to {vm_ip} ({engine.upper()})..."
                )
                ...
                if is_success:
                    ...
                    notes = f"Tested successfully on Proxmox {mode.upper()} ({engine})."
                    ...
                    logger.info(
                        f"Updated template status headers for {comp_id} to 'tested' "
                        f"(version: {version_to_record}, notes: {notes})"
                    )
                else:
...
def run_proxmox_tests(cli_args) -> int:
    ...
    if len(matrix) > 1:
        ...
        m, e = matrix[0]
        logger.info(f"STARTING TEST RUN: Mode={m.upper()} | Engine={e.upper()}")
    ...
    history.extend(results_summary)
