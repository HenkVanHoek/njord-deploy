...
def display_secret_scanning(alerts: List[Dict[str, Any]]) -> None:
    ...
    """
    sys.stdout.write(f"\n{'='*70}\n")
    sys.stdout.write(f"🔑 SECRET SCANNING ALERTS: {len(alerts)} found\n")
    sys.stdout.write(f"{'='*70}\n")
    if not alerts:
        sys.stdout.write("✅ No open secret scanning alerts found.\n")
        return

    for item in alerts:
        aid = int(item.get("number", 0))
        tname = str(
            item.get("type_name")
        ...
        st = str(item.get("state", "open")).upper()
        dt = str(item.get("created_at", ""))
...
def main() -> None:
    ...
    if args.json:
        safe_output = {
            ...
            "secret_scanning": results["secret_scanning"],
        }
